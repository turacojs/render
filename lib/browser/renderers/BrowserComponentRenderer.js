        ...
        * @param $container */
        value: function load(componentName, $container) {
            var promise = _Promise.resolve(this.factory.load(componentName, $container)).then(function (component) {
                ...
                var promiseMap = undefined;

                if (component.components && component.components.length) {
                    promiseMap = component.components.map(function (componentName) {
                        ...
                        return _Promise.resolve($componentElement._component);
                    });
                }

                return _Promise.all(promiseMap || []).then(function () {
                    if (component.load) {
